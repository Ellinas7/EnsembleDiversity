...
class recovery_rejection_rate(diversity_metric):
    """
    Recovery Rejection Rate: corrisponde alla percentuale di volte in cui, 
    dopo il reject del primo classificatore, anche il secondo rigetta senza 
    fornire alcuna risposta.
    
    Formula: N?? / (N?1 + N?0 + N??)
    
    Questa metrica quantifica quanto spesso entrambi i classificatori si 
    astengono, indicando situazioni di massima incertezza dove nemmeno il
    classificatore di backup riesce a fornire una predizione.
    ...
    def __init__(self):
        super().__init__("Recovery Rejection Rate")
    
    def _compute(self, predictions: np.ndarray, y_test: np.ndarray,
                 X_test: np.ndarray = None, model = None) -> float:
        """
        Calcola il recovery rejection rate per una coppia di classificatori in recovery block.
        ...
        Returns:
            La proporzione di doppi reject quando il primo fa reject: N?? / (N?1 + N?0 + N??)
        """
        # Verifica che ci siano esattamente 2 classificatori
        if predictions.shape[0] != 2:
            raise ValueError(f"Recovery rejection rate richiede esattamente 2 classificatori, "
        ...
        return N_question_question / denominator