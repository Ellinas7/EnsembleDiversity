...
class recovery_failure_rate(diversity_metric):
    """
    Recovery Failure Rate: misura la percentuale di risposte sbagliate date 
    dal secondo classificatore a seguito di una rejection fatta dal primo 
    classificatore.
    
    Formula: N?0 / (N?1 + N?0 + N??)
    
    Questa metrica quantifica quanto spesso il tentativo di recovery fallisce,
    con il secondo classificatore che fornisce una predizione errata quando
    viene consultato dopo la rejection del primario.
    ...
    def __init__(self):
        super().__init__("Recovery Failure Rate")
    
    def _compute(self, predictions: np.ndarray, y_test: np.ndarray,
                 X_test: np.ndarray = None, model = None) -> float:
        """
        Calcola il recovery failure rate per una coppia di classificatori in recovery block.
        ...
        Returns:
            La proporzione di risposte sbagliate del secondo classificatore
            quando il primo fa reject: N?0 / (N?1 + N?0 + N??)
        """
        # Verifica che ci siano esattamente 2 classificatori
        if predictions.shape[0] != 2:
            raise ValueError(f"Recovery failure rate richiede esattamente 2 classificatori, "
        ...
        return N_question_0 / denominator