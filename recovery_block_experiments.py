...
OUTPUT_FILE = "/Users/matteopascuzzo/Desktop/RecoveryBlock.csv"
...
# Genera entrambe le direzioni per ogni coppia
PAIRS = []
for a, b in BASE_PAIRS:
    PAIRS.append((a, b))  # direzione originale
    PAIRS.append((b, a))  # direzione inversa
...
def get_algorithms_needed() -> set:
    """Restituisce il set di algoritmi necessari per le coppie"""
    needed = set()
...
def run_experiment(ds, trained_classifiers: dict, pair: tuple, rejection_name: str, 
    ...
    clf_primary = rejection_func(trained_classifiers[primary_name])
    clf_secondary = rejection_func(trained_classifiers[secondary_name])
    
    # Crea ensemble (ordine: primario, secondario)
    ensemble = RecoveryBlockEnsemble([clf_primary, clf_secondary])
    ...
    experiment_name = f"{primary_name}->{secondary_name}"
    
    results = {
        "experiment_name": experiment_name,
        "dataset_name": ds.dataset_name,
        "ensemble_type": "RecoveryBlock",
        "rejection_strategy": rejection_name,
        "primary_classifier": primary_name,
        "secondary_classifier": secondary_name
    }
...
def main():
    print("=" * 70)
    print("RECOVERY BLOCK - EXPERIMENT RUNNER")
    print("=" * 70)
    print(f"Dataset: {len(DATASETS)}")
    print(f"Coppie (con entrambe le direzioni): {len(PAIRS)}")
